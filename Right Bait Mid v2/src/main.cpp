  ...
  while (timer::system() - startTime < 1300) {
    ...
    if (fabs(pitch) > 5) {
      ...
      if (fabs(pitch) > stopAng) {
        ...
        drive(speed, speed, 10); // Back up bc it has already overshot
      }
...
void auton() {
  ...
  unitDrive(0.75, 1, 1, 1000, 101); // get it
  turnTo(-45,875);
  unitDrive(1.3 * DIAG,1,0,INF,101,false,0,YELLOW);
  turnTo(-45,333); // fix direction
  unitDrive(-1.3 * DIAG, false, 0, INF, 101, true, 8);
  // ALLIANCE
  turnTo(-80); // fix direction
  mogoTilt(TILT_OPEN); // open tilt
  VisionBack.takeSnapshot(MOGO_BLUE); // determine which side we're on
  Rings.setVelocity(100, pct);
  Rings.spinFor(100, msec);
  ...
  unitDrive(-1.5,false,0,INF,101); // go back home
}
