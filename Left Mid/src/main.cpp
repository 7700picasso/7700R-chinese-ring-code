...
void auton() {
  ...
  unitDrive(2,1,1,1000,INF,false,0,YELLOW,true);
  Lift.resetPosition();
  Vision.takeSnapshot(YELLOW);
  unitDrive(-1,false,0,INF,INF,true,5,0,true,2);
  if (Vision.largestObject.exists && Vision.largestObject.width > 100) {
    turnTo(15,667);
    unitDrive(-2, 0, 0, 2000,INF,false,0,0,true); // back up. This may take a while if we're playing tuggle war. We have plenty of time at this point.
    return;
  }
  else {
    Claw(CLAW_OPEN);
    ...
    turnTo(-67,750);
    rings(true);
    unitDrive(1,1,1,INF, INF, false,0,YELLOW,true); // get side
    unitDrive(-3,1,1,INF,INF,true,8);
    return;
  }
  /*// ALLIANCE
